<div class="btn-group " role="group" aria-label="Acciones de gestión">
    ...
    data-url="{{ route('usuarios.update', $user->id) }}"
    data-name="{{ $user->name }}"
    data-lastname="{{ $user->last_name }}"
    data-cedula="{{ $user->cedula }}"
    data-email="{{ $user->email }}"
    ...
    <form action="{{ route('usuarios.destroy', $user->id) }}" method="POST" class="m-0 p-0 d-inline-block" onsubmit="return confirm('¿Estás totalmente seguro de eliminar a este profesor del sistema?');">
        @csrf
        @method('DELETE')
